...
// Filters
export const inMonth = createInMonthSelectors(arraySelector(TransactionResource), t =>
  beginningOfMonth(t.date)
);
export const upToMonth = createUpToMonthSelectors(arraySelector(TransactionResource), t =>
  beginningOfMonth(t.date)
);

// Map
export const getPayees = createSelector(arraySelector(TransactionResource), transactions => [
  ...new Set(transactions.filter(t => t.payee != null).map(t => t.payee))
]);

// Grouping
export const getToBeBudgetedSumUpToSelectedMonth = createSelector(upToMonth.current, transactions =>
