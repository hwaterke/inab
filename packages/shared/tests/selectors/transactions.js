...
export function transactionsSelectorTests(getStore: Function) {
  describe('Transactions selectors', () => {
    ...
    test('upToMonth', () => {
      ...
      expect(transactionsUpToMonth.previous(getStore().getState())).toEqual([
        t1,
      ])
      expect(transactionsUpToMonth.selected(getStore().getState())).toEqual([
        t1,
        t2,
      ])
      expect(transactionsUpToMonth.next(getStore().getState())).toEqual([
        t1,
        t2,
        t3,
      ])
    ...
    test('getToBeBudgetedSumUpToSelectedMonth', () => {
      ...
      expect(
        getToBeBudgetedSumUpToSelectedMonth(getStore().getState())
      ).toEqual(8)
    })
    ...
    test('selectSelectedMonthActivityByCategoryId', () => {
      ...
      getStore().dispatch(selectMonth(2017, 5))
      expect(
        selectSelectedMonthActivityByCategoryId(getStore().getState())
      ).toEqual({
        cat1: -8,
        cat2: -7,
      })
