...
export function budgetItemsSelectorTests(getStore) {
  describe('Budget Items selectors', () => {
    ...
    test('upToMonth', () => {
      ...
      expect(budgetItemsUpToMonth.previous(getStore().getState())).toEqual([
        bi1,
      ])
      expect(budgetItemsUpToMonth.selected(getStore().getState())).toEqual([
        bi1,
        bi2,
      ])
      expect(budgetItemsUpToMonth.next(getStore().getState())).toEqual([
        bi1,
        bi2,
        bi3,
      ])
    ...
    test('getSelectedMonthBudgetItemByCategoryId', () => {
      ...
      expect(
        getSelectedMonthBudgetItemByCategoryId(getStore().getState())
      ).toEqual({
        c1: bi1,
        c2: bi2,
      })
    ...
    test('getBudgetItemsSumUpToPreviousMonth', () => {
      ...
      expect(getBudgetItemsSumUpToPreviousMonth(getStore().getState())).toEqual(
        8
      )
    })
  })
