...
export function budgetUseCaseTests(getStore: Function) {
  describe('Budget selectors', () => {
    ...
    const expectAvailableByCategoryIdForSelectedMonth = (month, value) => {
      ...
      expect(
        getAvailableByCategoryIdForSelectedMonth(getStore().getState())
      ).toEqual(value)
    }
    ...
    describe('Handle default state', function () {
      ...
      describe('#getAvailableToBudget', function () {
        it('Should be 0 for previous month', () =>
          expectAvailable(getMonth(-1), 0))
        it('Should be 0 for current month', () =>
          expectAvailable(getMonth(), 0))
    ...
    describe('Handle one inflow', function () {
      ...
      describe('#getAvailableToBudget', function () {
        it('Should ignore inflow of next month', () =>
          expectAvailable(getMonth(-1), 0))
        it('Should include inflow of current month', () =>
          expectAvailable(getMonth(), 3))
        it('Should include left over from last month', () =>
      ...
      describe('#getFundsForSelectedMonth', function () {
        ...
        it('Should include inflow of current month', () =>
          expectFunds(getMonth(), 3))
        it('Should include left over from last month', () =>
          expectFunds(getMonth(1), 3))
      })
    })
    ...
    describe('Multiple inflows', function () {
      ...
      describe('#getAvailableToBudget', function () {
        it('Should ignore inflow of next months', () =>
          expectAvailable(getMonth(-1), 3))
        it('Should combine inflow of current and previous month', () =>
          expectAvailable(getMonth(), 8))
        it('Should combine inflows over one month', () =>
          expectAvailable(getMonth(1), 15))
      })
      describe('#getFundsForSelectedMonth', function () {
        it('Should ignore inflow of next month', () =>
          expectFunds(getMonth(-1), 3))
        it('Should combine inflow of current and previous month', () =>
          expectFunds(getMonth(), 8))
        it('Should combine inflows over one month', () =>
          expectFunds(getMonth(1), 15))
      })
    })
    ...
    describe('Use less than last month remaining funds', function () {
      ...
      describe('#getAvailableToBudget', function () {
        it('Should account for future budgeting', () =>
          expectAvailable(getMonth(-1), 2))
        it('Should account for budgetting in current month', () =>
          expectAvailable(getMonth(), 9))
        it('Should account for previous budgeting', () =>
          expectAvailable(getMonth(1), 22))
      })
      describe('#getFundsForSelectedMonth', function () {
        it('should ignore future budgeting', () => expectFunds(getMonth(-1), 5))
        it('should ignore budgeting of the month', () =>
          expectFunds(getMonth(), 5 + 7))
        it('should include past budgeting', () =>
          expectFunds(getMonth(1), 5 + 7 - 3 + 13))
      })
    })
    ...
    describe('Use all of last month remaining funds', function () {
      ...
      describe('#getAvailableToBudget', function () {
        it('Should account for future budgeting', () =>
          expectAvailable(getMonth(-1), 0))
        it('Should account for budgetting in current month', () =>
          expectAvailable(getMonth(), 7))
        it('Should account for previous budgeting', () =>
          expectAvailable(getMonth(1), 20))
      })
      describe('#getFundsForSelectedMonth', function () {
        it('should ignore future budgeting', () => expectFunds(getMonth(-1), 5))
        it('should ignore budgeting of the month', () =>
          expectFunds(getMonth(), 5 + 7))
        it('should include past budgeting', () =>
          expectFunds(getMonth(1), 5 + 7 - 5 + 13))
      })
    })
    ...
    describe('Use more than last month remaining funds', function () {
      ...
      describe('#getAvailableToBudget', function () {
        it('Should account for future budgeting', () =>
          expectAvailable(getMonth(-1), 0))
        it('Should account for budgetting in current month', () =>
          expectAvailable(getMonth(), 4))
        it('Should account for previous budgeting', () =>
          expectAvailable(getMonth(1), 17))
      })
      describe('#getFundsForSelectedMonth', function () {
        it('should ignore future budgeting', () => expectFunds(getMonth(-1), 5))
        it('should ignore budgeting of the month', () =>
          expectFunds(getMonth(), 5 + 7))
        it('should include past budgeting', () =>
          expectFunds(getMonth(1), 5 + 7 - 8 + 13))
      })
    })
    ...
    describe('Over budgeting', function () {
      ...
      describe('#getFundsForSelectedMonth', function () {
        ...
        it('should be available next month', () =>
          expectFunds(getMonth(1), -35))
      })

      describe('#getOverspentLastMonth', function () {
        it('should not affect previous month', () =>
          expectOverspentLastMonth(getMonth(-1), 0))
        it('should not affect current month', () =>
          expectOverspentLastMonth(getMonth(), 0))
        it('should not affect next month', () =>
          expectOverspentLastMonth(getMonth(1), 0))
      })

      describe('#getBudgetedThisMonth', function () {
        it('should not affect previous month', () =>
          expectBudgetedThisMonth(getMonth(-1), 0))
        it('should affect current month', () =>
          expectBudgetedThisMonth(getMonth(), -50))
        it('should not affect next month', () =>
          expectBudgetedThisMonth(getMonth(1), 0))
      })

      describe('#getBudgetedInFuture', function () {
        it('should affect previous month', () =>
          expectBudgetedInFuture(getMonth(-1), -3))
        it('should not affect current month', () =>
          expectBudgetedInFuture(getMonth(), 0))
        it('should not affect next month', () =>
          expectBudgetedInFuture(getMonth(1), 0))
      })
    ...
    describe('Overspending', function () {
      ...
      describe('#getFundsForSelectedMonth', function () {
        it('should be default in previous month', () =>
          expectFunds(getMonth(-1), 0))
        it('should not be affected by current month overspending', () =>
          expectFunds(getMonth(), 0))
        it('should not be affected by previous oversepnding', () =>
          expectFunds(getMonth(1), 0))
    ...
    describe('Unbudgeting', function () {
      ...
      describe('#getOverspentLastMonth', function () {
        ...
        it('should not affect current month', () =>
          expectOverspentLastMonth(getMonth(), 0))
        it('should be reflected next month', () =>
          expectOverspentLastMonth(getMonth(1), -3))
      })
      ...
      describe('#getBudgetedThisMonth', function () {
        it('should not affect previous month', () =>
          expectBudgetedThisMonth(getMonth(-1), 0))
        it('should affect current month', () =>
          expectBudgetedThisMonth(getMonth(), 3))
        it('should not affect next month', () =>
          expectBudgetedThisMonth(getMonth(1), 0))
      })
