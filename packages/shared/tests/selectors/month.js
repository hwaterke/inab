...
export function monthSelectorTests(getStore) {
  describe('Month selectors', () => {
    test('getSelectedMonthMoment', () => {
      ...
      expect(
        getSelectedMonthMoment(getStore().getState()).format(
          'YYYY-MM-DD HH:mm:ss'
        )
    ...
    test('getPreviousMonthMoment', () => {
      getStore().dispatch(selectMonth(2017, 0))
      expect(
        getPreviousMonthMoment(getStore().getState()).format(
          'YYYY-MM-DD HH:mm:ss'
        )
      ).toEqual('2016-12-01 00:00:00')
    })

    test('getNextMonthMoment', () => {
      getStore().dispatch(selectMonth(2017, 0))
      expect(
        getNextMonthMoment(getStore().getState()).format('YYYY-MM-DD HH:mm:ss')
